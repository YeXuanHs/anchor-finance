<template>
  <div class="users-page">
      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            ...
            />
            <el-button type="primary" :icon="Plus" @click="openModal()">添加用户</el-button>
          </div>
        </div>
      </template>

      <el-table :data="paginatedUsers" style="width: 100%" v-loading="loading" size="default">
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            ...
            <el-button type="primary" link :icon="Edit" @click="openModal(row)">编辑</el-button>
            <el-popconfirm
              title="确认删除该用户？"
    ...
    <!-- User Detail Drawer -->
    <el-drawer v-model="drawerVisible" :title="drawerUser?.username || '用户详情'" size="480px">
      <template v-if="drawerUser">
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item label="余额">
            <span style="color: #409eff; font-weight: 600">¥{{ drawerUser.balance.toFixed(2) }}</span>
        ...
        <el-table :data="drawerOrders" size="small" style="width: 100%">
          ...
          <el-table-column prop="status" label="状态" width="80">
            <template #default="{ row }">
              <el-tag :type="drawerOrderStatusMap[row.status]?.type" size="small">
