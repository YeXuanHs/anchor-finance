...
# 获取字段列表
cmd = f'curl -s "http://localhost:8080/api/admin/custom-fields" -H "Authorization: Bearer {token}"'
stdin, stdout, stderr = client.exec_command(cmd)
...
if data.get('code') == 0:
    ...
    print(f"  字段列表: FAIL - {data.get('message')}")
...
data = {
    ...
    "label": "公司规模",
    "type": "select",
    "options": '[{"value":"small","label":"小型"},{"value":"medium","label":"中型"},{"value":"large","label":"大型"}]',
    "required": True
}
cmd = f'curl -s -X POST "http://localhost:8080/api/admin/custom-fields" -H "Authorization: Bearer {token}" -H "Content-Type: application/json" -d \'{json.dumps(data)}\''
stdin, stdout, stderr = client.exec_command(cmd)
result = stdout.read().decode('utf-8', errors='ignore')
...
if resp.get('code') == 0:
    ...
    print(f"  创建字段: FAIL - {resp.get('message')}")
