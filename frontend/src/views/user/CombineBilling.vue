...
const fetchInvoices = async () => {
  loading.value = true
  try {
    const { data } = await request.get('/api/user/invoices', { params: { status: 'unpaid', page_size: 100 } })
...
const handleCombine = async () => {
  ...
  try {
    ...
    const { data } = await request.post('/api/user/invoices/combine', { invoice_ids: ids })
