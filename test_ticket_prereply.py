...
# 获取预回复列表
cmd = f'curl -s "http://localhost:8080/api/admin/ticket-prereplies" -H "Authorization: Bearer {token}"'
stdin, stdout, stderr = client.exec_command(cmd)
...
if data.get('code') == 0:
    ...
    print(f"  预回复列表: FAIL - {data.get('message')}")

# 获取预回复分类
cmd = f'curl -s "http://localhost:8080/api/admin/ticket-prereply-categories" -H "Authorization: Bearer {token}"'
stdin, stdout, stderr = client.exec_command(cmd)
result = stdout.read().decode('utf-8', errors='ignore')
data = json.loads(result)
if data.get('code') == 0:
    ...
    print(f"  预回复分类: FAIL - {data.get('message')}")
