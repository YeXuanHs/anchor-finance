...
func (h *BalanceHandler) GetWithdrawals(c *gin.Context) {
	...
	var total int64
	h.db.Model(&Withdrawal{}).Count(&total)
	h.db.Offset((page - 1) * pageSize).Limit(pageSize).Order("id DESC").Find(&withdrawals)
...
func (h *BalanceHandler) ApproveWithdrawal(c *gin.Context) {
	...
	h.db.Model(&w).Update("status", 1)
	c.JSON(http.StatusOK, gin.H{"message": "approved"})
}

// RejectWithdrawal 拒绝提现
func (h *BalanceHandler) RejectWithdrawal(c *gin.Context) {
	id, _ := strconv.Atoi(c.Param("id"))
	var w Withdrawal
	if err := h.db.First(&w, id).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "withdrawal not found"})
	...
	h.db.Model(&w).Updates(map[string]interface{}{"status": 2, "remark": req.Remark})
