...
apis = [
    ...
    ("POST", "/api/admin/staff", "创建员工", '{"username":"staff1","email":"staff1@test.com","password":"123456","real_name":"测试员工"}'),
]

print("=== 测试员工管理API ===")
for item in apis:
    method, path, desc = item[0], item[1], item[2]
    data = item[3] if len(item) > 3 else None
    
    if method == "GET":
        cmd = f'curl -s "http://localhost:8080{path}" -H "Authorization: Bearer {token}"'
    else:
        cmd = f'curl -s -X {method} "http://localhost:8080{path}" -H "Authorization: Bearer {token}" -H "Content-Type: application/json" -d \'{data}\''
    ...
    try:
        ...
        if resp.get('code') == 0:
            ...
            print(f"  {desc}: FAIL - {resp.get('message')}")
    except:
        print(f"  {desc}: FAIL")
