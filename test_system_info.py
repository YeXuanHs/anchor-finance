...
# 获取系统信息
cmd = f'curl -s "http://localhost:8080/api/admin/system/info" -H "Authorization: Bearer {token}"'
stdin, stdout, stderr = client.exec_command(cmd)
result = stdout.read().decode('utf-8', errors='ignore')
data = json.loads(result)
if data.get('code') == 0:
    print("  系统信息: OK")
    print(f"    数据库大小: {data['data']['database']['size_mb']} MB")
    print(f"    表数量: {data['data']['database']['table_count']}")
    print(f"    Go版本: {data['data']['server']['go_version']}")
else:
    print(f"  系统信息: FAIL - {data.get('message')}")

# 获取系统模块
cmd = f'curl -s "http://localhost:8080/api/admin/system/modules" -H "Authorization: Bearer {token}"'
stdin, stdout, stderr = client.exec_command(cmd)
result = stdout.read().decode('utf-8', errors='ignore')
data = json.loads(result)
if data.get('code') == 0:
    print(f"  系统模块: OK ({len(data['data'])}个模块)")
else:
    print(f"  系统模块: FAIL - {data.get('message')}")
