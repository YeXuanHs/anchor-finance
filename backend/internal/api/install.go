...
func createDefaultConfigs(dbConn *gorm.DB, siteName, siteURL string, req installRequest) error {
	siteDesc := req.SiteDescription
	if siteDesc == "" {
		siteDesc = "高效、安全的财务管理平台"
	}
	...
	defaults := []systemSetting{
		...
		{Key: "site_description", Value: siteDesc, Group: "general", Desc: "站点描述"},
		...
		{Key: "admin_path", Value: adminPath, Group: "general", Desc: "后台访问路径"},
...
# Auto-generated by installer - 仅包含连接信息和基础配置
...
func randomSecret(n int) string {
	...
	for i := range b {
		...
		if err != nil {
			// Fallback: use a deterministic but safe-ish approach
