...
type TicketDepartment struct {
	...
	ManagerIDs     datatypes.JSON `gorm:"type:jsonb" json:"manager_ids"` // 负责人ID列表 [1,2,3]
	MemberIDs      datatypes.JSON `gorm:"type:jsonb" json:"member_ids"`  // 成员ID列表
	...
	AssignRule     datatypes.JSON `gorm:"type:jsonb" json:"assign_rule"` // 自动分配规则：round_robin(轮询)/least_load(最少工单)/random(随机)
