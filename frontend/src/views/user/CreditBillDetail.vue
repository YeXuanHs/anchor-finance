...
async function fetchBillDetail() {
  ...
  loading.value = true
  try {
    const res = await request.get(`/api/v2/credit/bills/${id}`)
...
async function fetchRepayRecords() {
  ...
  if (!id) return
  try {
    const res = await request.get(`/api/v2/credit/bills/${id}/repayments`)
...
async function handleRepay() {
  ...
  try {
    await request.post(`/api/v2/credit/bills/${route.params.id}/pay`, { amount: repayAmount.value })
