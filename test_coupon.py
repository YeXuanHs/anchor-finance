...
# 获取优惠券列表
cmd = f'curl -s "http://localhost:8080/api/admin/coupons" -H "Authorization: Bearer {token}"'
stdin, stdout, stderr = client.exec_command(cmd)
...
if data.get('code') == 0:
    ...
    print(f"  优惠券列表: FAIL - {data.get('message')}")

# 获取优惠券统计
cmd = f'curl -s "http://localhost:8080/api/admin/coupons/summary" -H "Authorization: Bearer {token}"'
stdin, stdout, stderr = client.exec_command(cmd)
result = stdout.read().decode('utf-8', errors='ignore')
data = json.loads(result)
if data.get('code') == 0:
    ...
    print(f"  优惠券统计: FAIL - {data.get('message')}")
...
cmd = f'curl -s -X POST "http://localhost:8080/api/admin/coupons" -H "Authorization: Bearer {token}" -H "Content-Type: application/json" -d \'{json.dumps(coupon_data)}\''
stdin, stdout, stderr = client.exec_command(cmd)
result = stdout.read().decode('utf-8', errors='ignore')
data = json.loads(result)
if data.get('code') == 0:
    ...
    print(f"  创建优惠券: FAIL - {data.get('message')}")
