...
type Rule struct {
	...
	Condition  datatypes.JSON `gorm:"type:jsonb;not null" json:"condition"`           // 条件JSON
	Action     datatypes.JSON `gorm:"type:jsonb;not null" json:"action"`              // 动作JSON
...
type RuleLog struct {
	...
	MatchData datatypes.JSON `gorm:"type:jsonb" json:"match_data"`               // 匹配时的输入数据
	Result    datatypes.JSON `gorm:"type:jsonb" json:"result"`                   // 执行结果
	Actions   datatypes.JSON `gorm:"type:jsonb" json:"actions"`                  // 实际执行的动作
