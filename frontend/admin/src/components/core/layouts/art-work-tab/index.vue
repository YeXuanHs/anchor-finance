    ...
    <div class="w-full overflow-hidden" ref="scrollRef">
      ...
      >
        ...
        >
          ...
          />
          {{ item.customTitle || getMenuTitle({ title: item.title, languageMap: item.languageMap }, locale) }}
          <span
...
<script setup lang="ts">
  ...
  import { getMenuTitle } from '@/utils/router'
  ...
  // 更新标签页的 languageMap
  const updateTabsLanguageMap = () => {
    const routes = appRouter.getRoutes()
    list.value.forEach((tab) => {
      if (!tab.languageMap) {
        const matchedRoute = routes.find((r) => r.path === tab.path)
        if (matchedRoute?.meta?.languageMap) {
          tab.languageMap = matchedRoute.meta.languageMap as Record<string, string>
        }
      }
    })
  }
